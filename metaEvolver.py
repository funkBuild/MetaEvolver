...
mod = SourceModule("""
  ...
  __shared__ float sharedTree[64];
  ...
  __global__ void calc(float *tree, float *data , bool *winnerTable, int datalength)
  {
	...
	//if(2 * threadIdx.y < 127 ){
	//	sharedTree[threadIdx.y] = tree[treeIndex + 2*threadIdx.y];
	//};
  ...
  __global__ void fitness(float *data, bool *winnerTable, int *winCounter, int *lossCounter, int *winTrades, int *lossTrades, int datalength)
  {
	...
	const int takeProfit = 10;
	const int stopLoss = -10;
	const int maxTimeOpen = 240;
	...
	for(int x = 0; x < datalength-1; x++){
		...
		if(tradeOpen){
			timeOpen++;
			deltaHigh =  ((data[46*x + 2] - open ) * 10000.0);
			deltaLow =  ((data[46*x + 3] - open ) * 10000.0);

			if(deltaHigh > takeProfit || deltaLow < stopLoss || x >= datalength-2 || timeOpen >= maxTimeOpen ){
				if(timeOpen >= maxTimeOpen){
					float delta = rintf((data[46*x + 4] - open ) * 10000.0);
					
					if(delta < 0) {

						int *loseCount = &lossCounter[counterOffset + x / kfolddivider];
						if(deltaLow <= stopLoss) deltaLow = stopLoss;
			    			atomicAdd(loseCount, -deltaLow ); 
						...
						if(deltaHigh >= takeProfit) deltaHigh = takeProfit;
			    			atomicAdd(winCount, deltaHigh );
...
endDate = datetime.datetime(2011, 9, 1, 0, 0)
	...
	while True:
		...
		if lastTrees:
			...
			for x in range( poolSize ):
				...
				evalArray.append( [totalExpectancy, round(wins + losses, 0), wonPips - lostPips, wonPips, lostPips, 0] )	
	 		#print evalArray
