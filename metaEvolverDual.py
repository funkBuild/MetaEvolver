...
mod = SourceModule("""
  ...
  const int treelength = 15;  // 2^n - 1
  #define BINS 12;

  __shared__ float sharedTree[treelength];
  ...
  __device__ float getFloatNodeValue(float *data, int offset, const int dataIdx){
	if(offset > treelength) return 0.0;
  ...
  __global__ void calc(float *tree, float *data , bool *winnerTable, int datalength)
  {
	...
	if(thredIdx <= 46 || dataIdx >= 46*datalength) return;
  ...
  __global__ void fitness(float *data, bool *winnerTable, int *winCounter, int *lossCounter, int *winTrades,int *lossTrades, int datalength)
  {
	const int thredIdx = (blockIdx.y*blockDim.y + threadIdx.y);
	...
	const int takeProfit = 20;
	const int stopLoss = -20;
	const int maxTimeOpen = 120;
	...
	for(int x = 0; x < datalength-1; x++){
		...
		//if there isn't a trade open and the winner table goes from false to true then open a trade
		if(!tradeOpen && (winnerTable[winnerOffset + x] && !winnerTable[winnerOffset + x -1])){
			open = data[46*x + 4] + 0.0002;
			tradeOpen = true;
		...
		if(tradeOpen){
			timeOpen++;
			delta =  rintf((data[46*x + 4] - open ) * 10000.0);

			if(delta > takeProfit || delta < stopLoss || x >= datalength-2 || timeOpen >= maxTimeOpen ){
				if(delta == 0){}
				else if(delta > 0){
					int *winCount = &winCounter[counterOffset + x / kfolddivider];

					if(delta >= takeProfit) delta = takeProfit;
					...
					if(delta <= stopLoss) delta = stopLoss;
...
endDate = datetime.datetime(2012, 1, 1, 0, 0)
primExchange = 'AUDUSD'
secExchange = 'EURUSD'

data = []

for dataPoint in dataCollection.find({'datetime': {'$gt': startDate , '$lt': endDate }, 'ticker': primExchange }).sort("datetime"):
	data.append( dataPoint["data"] )

dataSec = []

for dataPoint in dataCollection.find({'datetime': {'$gt': startDate , '$lt': endDate }, 'ticker': secExchange }).sort("datetime"):
...
while True:
	...
	dataDimSec = math.floor(len(dataSec)/64.0)
	...
	winnerTableSec = numpy.zeros(len(data) * poolSize, dtype=numpy.bool)
	winnerTableSec_gpu = cuda.mem_alloc(winnerTableSec.nbytes)

	winCounterSec = numpy.zeros(12 * poolSize, dtype=numpy.int32)
	winCounterSec_gpu = cuda.mem_alloc(winCounterSec.nbytes)

	lossCounterSec = numpy.zeros(12 * poolSize, dtype=numpy.int32)
	lossCounterSec_gpu = cuda.mem_alloc(lossCounterSec.nbytes)

	winTradesSec = numpy.zeros(12 * poolSize, dtype=numpy.int32)
	winTradesSec_gpu = cuda.mem_alloc(winTradesSec.nbytes)

	lossTradesSec = numpy.zeros(12 * poolSize, dtype=numpy.int32)
	lossTradesSec_gpu = cuda.mem_alloc(lossTradesSec.nbytes)
	...
	while True:
		...
		winnerTableSec = numpy.zeros(len(data) * poolSize, dtype=numpy.bool)
		cuda.memcpy_htod(winnerTableSec_gpu, winnerTableSec)

		winCounterSec = numpy.zeros(12 * poolSize, dtype=numpy.int32)
		cuda.memcpy_htod(winCounterSec_gpu, winCounterSec)

		lossCounteSecr = numpy.zeros(12 * poolSize, dtype=numpy.int32)
		cuda.memcpy_htod(lossCounterSec_gpu, lossCounterSec)

		winTradesSec = numpy.zeros(12 * poolSize, dtype=numpy.int32)
		cuda.memcpy_htod(winTradesSec_gpu, winTradesSec)

		lossTradesSec = numpy.zeros(12 * poolSize, dtype=numpy.int32)
		cuda.memcpy_htod(lossTradesSec_gpu, lossTradesSec)
		...
		fitness(data_gpu, winnerTable_gpu, winCounter_gpu, lossCounter_gpu, winTrades_gpu, lossTrades_gpu, numpy.int32(len(data)), block=(1, 1, 1), grid=(poolSize,1)) 

		# Secondary calc
		calc(trees_gpu, dataSec_gpu, winnerTableSec_gpu, numpy.int32(len(dataSec)), block=(1, 64, 1), grid=(poolSize,int(dataDimSec))) # x is the tree index, y is the data index
		fitness(dataSec_gpu, winnerTableSec_gpu, winCounterSec_gpu, lossCounterSec_gpu, winTradesSec_gpu, lossTradesSec_gpu, numpy.int32(len(dataSec)), block=(1, 1, 1), grid=(poolSize,1)) 
		...
		if lastTrees:
			...
			for x in range( poolSize ):
				...
				nanCount = 0
				for y in range( 12 ):
					expect = expectancy(winsArray[y], lossesArray[y], winArray[y], loseArray[y]);
					if( math.isnan(expect) ):
						expect = 0;
			
					expectArray.append( expect )

				totalExpectancy = expectancy(wins, losses, wonPips, lostPips)


				# Secondary

				winArraySec = winCountSec[x*12: (x+1)*12]
				loseArraySec = lossCountSec[x*12: (x+1)*12]

				winsArraySec = winTradeCountSec[x*12: (x+1)*12]
				lossesArraySec = lossTradeCountSec[x*12: (x+1)*12]

				wonPipsSec = sum(winArraySec)
				lostPipsSec = sum(loseArraySec)

				winsSec = sum( winsArraySec )
				lossesSec = sum( lossesArraySec )
				...
				for y in range( 12 ):
					...
					expectArraySec.append( expect )

				totalExpectancySec = expectancy(winsSec, lossesSec, wonPipsSec, lostPipsSec)


				evalArray.append( [totalExpectancy, wins + losses, wonPips - lostPips, wonPips, lostPips, 0] )	
	 		#print evalArray
		...
		winCountSec = numpy.empty_like(winCounterSec)
		cuda.memcpy_dtoh(winCountSec, winCounterSec_gpu)

		lossCountSec = numpy.empty_like(lossCounterSec)
		cuda.memcpy_dtoh(lossCountSec, lossCounterSec_gpu)

		winTradeCountSec = numpy.empty_like(winTradesSec)
		cuda.memcpy_dtoh(winTradeCountSec, winTradesSec_gpu)

		lossTradeCountSec = numpy.empty_like(lossTradesSec)
		cuda.memcpy_dtoh(lossTradeCountSec, lossTradesSec_gpu)
