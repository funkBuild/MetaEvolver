...
mod = SourceModule("""
  ...
  __global__ void fitness(float *data, bool *winnerTable, int *winCounter, int *lossCounter,int *lossTrades, int *winTrades, int datalength)
  {
	...
	const int takeProfit = 125;
	const int stopLoss = -75;
	...
	if(winnerTable[winnerOffset + thredIdx] && !winnerTable[winnerOffset + thredIdx -1]){
		...
		for(int x = thredIdx; x < datalength-1; x++){
			...
			if(delta > takeProfit || delta < stopLoss || x >= datalength-2){
				...
				if(delta == 0){}
					...
					if(delta > takeProfit) delta = takeProfit;
		    			atomicAdd(winCount, delta ); // delta
					...
					if(delta < stopLoss) delta = stopLoss;
		    			atomicAdd(loseCount, -delta ); // -delta
