...
genomes = genomeDatabase.find({'expectancy': {"$lt": 5}, 'orderType': 'sell'  }).sort('expectancy', -1)
genomesCount = genomes.count()
print "Total Genomes", genomesCount
...
while True:
	...
	for x in range( len(trees) ):
		...
		winnerTableArray = winnerTable[x*dataLength: (x+1)*dataLength]
		...
		displayX = []
		displayY = []
		for y in range(len(winnerTableArray)):
			if not isTrading and winnerTableArray[y]:
				isTrading = True
			elif isTrading and not winnerTableArray[y]:
				isTrading = False
				balanceTotal +=  balanceArray[y]
				displayX.append(y)
				displayY.append(balanceTotal)
				trades += 1

		wonPips = sum(winArray)
		lostPips = sum(loseArray)

		print balanceTotal, wonPips - lostPips
		print trades
		...
		evalArray.append( [round(expect,2), 0, wonPips - lostPips, statArr[x]['expectancy'] ] )	
		if wonPips - lostPips > 0:
			plt.plot(displayX, displayY)
			plt.show()
	generations += 1
	print generations
