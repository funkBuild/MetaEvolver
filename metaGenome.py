	...
	for index in range( 1, treeSize ):
		...
		if index < (2**(size-2) - 1):			# if it's not the last or second last row
			...
			if parent < 1000:			# parent is a integer, child must be zero
				...
				if index % 1 == 0:
					tree[index] = random.randint(0,120)	# shift
				else:
					tree[index] = random.randint(0,45)	# variable
...
def checkDominance(fitness1, fitness2):  #check if the first genome dominate the second Pareto style
	if (fitness1[0] >= fitness2[0] and fitness1[1] > fitness2[1]) or (fitness1[0] > fitness2[0] and fitness1[1] >= fitness2[1]): # or (fitness1[0] >= fitness2[0] and fitness1[1] >= fitness2[1] and fitness1[2] > fitness2[2]): #  :		#if the first genomes fitness parameter isn't greater than the second then it doesn't dominate it
		return True
	else:
		return False
...
class Pool:
	def __init__(self, db, genomeType, ticker, trainDate):
	...
	def trimArchive(self):
		self.archive.sort(key=lambda a: a[1][2], reverse=False)
	...
	def saveCurrentGenome(self, genome):
		...
		self.genomeDatabase.insert({"parent_id": structure_id, "genome": genome[0], "expectancy": float(genome[1][0]), "trades": int(genome[1][1]), "wins": int(genome[1][3]), "losses": int(genome[1][4]), "drawdown": int(genome[1][2]), "orderType": self.genomeType, "trainDate": self.trainDate, "ticker": self.ticker })
		return
	...
	def processOptimizeMode(self, treesR, fitnessR):
		...
		for index in range( len (fitness) ):
			if math.isnan(fitness[index][0]) or fitness[index][1] > 250 :
				continue
			...
			#for x in toBlacklist:
			#	self.blacklist.add( getStructure(x[0]) )

			self.pool = [ x for x in self.pool if not checkDominance(fitness[index], x[1]) ]
		...
		if len(self.archive) > 750:
			self.trimArchive()

		self.plotArchive()

		print "Pool Genomes ", len (self.pool)
